...
public:
	...
	void set_color(rgb color, bool is_solid = true)
	{
		...
		pen = CreatePen(pen_mode, 1, RGB(color.rgb_r, color.rgb_g, color.rgb_b));
		SelectObject(hdc, pen);
		last_color = color;
		
		if (is_solid) {
			if (last_brush != color) {
				brush = CreateSolidBrush(RGB(color.rgb_r, color.rgb_g, color.rgb_b));
...
enum geo_object {geo_point, geo_color, geo_seg, geo_line, geo_circle, geo_quadratic};
...
public:
	...
	drawn_geo(
		geo_object p_obj, point_2d p_pos, rgb p_color, point_2d p_to = POINT_2D_ORIG, double p_param1 = 0.0,
		point_2d p_param2 = POINT_2D_ORIG)
	{
		obj = p_obj;
		pos = p_pos;
		to = p_to;
...
public:
	...
	inline void alpha_blend(int x, int y, float alpha = 0.5, rgb color = RGB_BLACK)
	{
		...
		write_pxl_safe(x, y, rgb(interpolate(alpha, exist.rgb_r, color.rgb_r), interpolate(alpha, exist.rgb_g, color.rgb_g), interpolate(alpha, exist.rgb_b, color.rgb_b)));
	}
	...
	{
		point_2dd tp = point_2dd(p.x, dims.y - p.y);
	...
	void disp_color(point_2d p, rgb c = RGB_RED, int radius = 3, bool scale = false, bool repos = true)
	{
		quantum_2d ctr = trans_point(p);
		
		for (int y = ctr.y - radius; y <= ctr.y + radius; y++) {
			for (int x = ctr.x - radius; x <= ctr.x + radius; x++) {
				color_point(x, y, c, 1.0, scale);
			}
		}
	}
	...
	void disp_seg(point_2d p1, point_2d p2, rgb color, bool scale = false)
	{
		point_2d t1 = trans_point(p1);
		point_2d t2 = trans_point(p2);

		double dx = double(t2.x - t1.x);
		double dy = double(t2.y - t1.y);
		...
		if (fabs(dx) > fabs(dy)) {
			...
			if (p1.x <= p2.x) {
				for (int x = int(t1.x); x <= int(t2.x); x++) {
					color_point(x, (int)floor(y), color, 1.0, scale);
					y += m;
				}
			} else {
				for (int x = int(t1.x); x >= int(t2.x); x--) {
			...
			if (t1.y <= t2.y) {
				for (int y = int(t1.y); y <= int(t2.y); y++) {
					color_point((int)x, y, color, 1.0, scale);
					x += m;
				}
			} else {
				for (int y = int(t1.y); y >= int(t2.y); y--) {
	...
	void disp_rectangle(int x1, int y1, int x2, int y2, rgb color)
	{
		for (int y = y1; y <= y2; y++)
			for (int x = x1; x <= x2; x++)
				color_point(x, y, color);
	}
	...
	/*
	void disp_quadratic(point_2d p1, point_2d p2, point_2d p3, rgb color = RGB_GREEN, bool scale = false)
	{
		point_2d tp1 = trans_point(p1);
		point_2d tp2 = trans_point(p2);
		point_2d tp3 = trans_point(p3);
		quadratic_2d<> quad(tp1, tp2, tp3);
		point_2d qp1, qp2;
		int x;
		int next_y;

		if (_isnan(quad.a)) return;

		if (approx_zero(quad.a)) {
			qp1 = tp1;
			qp2 = tp3;
		} else if (quad.a > 0) { // holds water
			qp1 = quad.trans_point2();
			qp2 = quad.trans_point1();
		} else { // umbrella
			qp1 = quad.trans_point1();
			qp2 = quad.trans_point2();
		}
		...
		if (approx_zero(quad.a)) {
			...
			if (tp1.x < qp1.x) {
				if (qp1.x > tp3.x) next_y = min(tp3.y, qp1.y);

				if (quad.a < 0.0) { // umbrella
					for (int y = tp1.y; y <= next_y; y++) {
						color_point((int)quad.inv_func_a(y), y, color, 1.0, false);
					}
				} else { // holds water
					if (qp1.x > tp3.x) next_y = max(tp3.y, qp1.y);

					for (int y = tp1.y; y >= next_y; y--) {
						color_point((int)quad.inv_func_b(y), y, color, 1.0, false);
					}
				}
				x = cap_max(dims.x - 1.0f, cap_min(0.0f, qp1.x));
			} else {
				x = tp1.x;
			}
		}

		if (!(approx_zero(quad.a) && fabs(quad.b) > 1.0)) {
			for (; x < min(tp3.x, cap_max(dims.x - 1.0f, cap_min(0.0f, qp2.x))); x++) {
				color_point(x, (int)quad.func(x), color, 1.0, false);
			}
		}

		if (approx_zero(quad.a)) {
			for (int y = min(tp1.y, tp3.y); y <= max(tp1.y, tp3.y); y++) {
				color_point((int)quad.inv_func_a(y), y, color, 1.0, false);
			}
		} else {
			next_y = qp2.y;
		...
		if (!approx_zero(quad.a)) {
			if (qp2.x < tp3.x) {
				if (quad.a < 0.0) { // umbrella
					if (tp1.x >= qp2.x) next_y = min(tp1.y, qp2.y);
				
					for (int y = next_y; y >= tp3.y; y--) {
						color_point((int)quad.inv_func_b(y), y, color, 1.0, false);
					}
				} else { // holds water
					if (tp1.x >= qp2.x) next_y = max(tp1.y, qp2.y);
	...
	*/
	...
	void disp_geometry(drawn_geo g)
	{
		if (g.obj == geo_point) {
			...
			disp_color(g.pos, g.color, int(g.param1));
		} else if (g.obj == geo_seg) {
			disp_seg(g.pos, g.to, g.color);
		...
		} else if (g.obj == geo_quadratic) {
			//disp_quadratic(g.pos, g.to, g.param2, g.color);
		}
	}
	...
	*/
	...
	void put_buffer()
	{
		...
		for (int y = 0; y < dims.y; y++) {
			...
			for (int x = 0; x < dims.x; x++) {
				...
				int u = int(floor(source.x));
				int v = int(floor(source.y));
	...
	{
		geometry.push_back(drawn_geo(geo_point, p, color, POINT_2D_ORIG, rad));
	}

	void log_color(point_2d p, double rad, rgb color)
	{
		geometry.push_back(drawn_geo(geo_color, p, color, POINT_2D_ORIG, rad));
	}

	void log_seg(point_2d p1, point_2d p2, rgb color)
	{
		geometry.push_back(drawn_geo(geo_seg, p1, color, p2, 0.0));
	}

	void log_line(point_2d p1, point_2d p2, rgb color)
	{
		line_2d_dydx l(p1, p2);
		geometry.push_back(drawn_geo(geo_line, p1, color, POINT_2D_ORIG, l.m));
	}

	void log_line(point_2d p, line_2d_dydx l, rgb color)
	{
		geometry.push_back(drawn_geo(geo_line, p, color, POINT_2D_ORIG, l.m));
	}

	void log_circle(circle_2d<> circ, rgb color)
	{
		geometry.push_back(drawn_geo(geo_circle, circ.center, color, POINT_2D_ORIG, circ.radius));
	}

	void log_quadratic(point_2d p1, point_2d p2, point_2d p3, rgb color)
	{
		geometry.push_back(drawn_geo(geo_quadratic, p1, color, p2, 0.0, p3));
	}
};
