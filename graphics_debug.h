...
void show_buffer()
{
	point_2d d = _test_ds[TEST_DS_OUT_GFX_1]->get_dims();

	_test_ds[TEST_DS_OUT_GFX_1]->put_buffer();
	show_windows(0);
	
	if (_test_win[TEST_WIN_OUT]->zoom > 1.0) {
		for (double x = 100 + (1.0 - frac(_test_ds[TEST_DS_OUT_GFX_1]->bot_lef.x)) * _test_win[TEST_WIN_OUT]->zoom; x < 100 + d.x; x += _test_win[TEST_WIN_OUT]->zoom) {
			_test_win[TEST_WIN_OUT]->draw_line(point_2d(int(x), 65), point_2d(int(x), 90));
		}

		for (double y = - 13 + 100 + (frac(_test_ds[TEST_DS_OUT_GFX_1]->bot_lef.y)) * _test_win[TEST_WIN_OUT]->zoom; y < 100 + d.y; y += _test_win[TEST_WIN_OUT]->zoom) {
...
void show_text(double n, char* s, int line_n = -1)
{
	if (skip_count < 0) {
		_test_win[TEST_WIN_TEXT_1]->put_text(n, s, line_n);
	}
...
void explore_outp(window& w1, window& w2, int exit_key = 27)
{
	...
	do {
		...
		if (k == int('C')) {
			...
			skip_count = (int)pow(10.0, double(k - int('0')));
		} else if (k == int('G')) {
			_test_ds[TEST_DS_OUT_GFX_1]->geo_on = !_test_ds[TEST_DS_OUT_GFX_1]->geo_on;
		} else if (k == int('B')) {
			_test_ds[TEST_DS_OUT_GFX_1]->back_on = !_test_ds[TEST_DS_OUT_GFX_1]->back_on;
		}
...
void show_point(int x, int y, rgb c = RGB_RED, int radius = 3, bool nav = true)
{
	_test_ds[TEST_DS_OUT_GFX_1]->log_point(point_2d(x, y), radius, c);

	if (nav) {
		_test_win[TEST_WIN_OUT]->cursor = point_2d(x, y);
		explore_outp(*_test_win[TEST_WIN_IN], *_test_win[TEST_WIN_OUT], 13);
	}
}

void show_color(point_2d p, rgb c, int radius = 3, bool nav = true)
{
	_test_ds[TEST_DS_OUT_GFX_1]->log_color(p, radius, c);
...
void show_seg(point_2d p1, point_2d p2, rgb c = RGB_RED, bool nav = true)
{
	_test_ds[TEST_DS_OUT_GFX_1]->log_seg(p1, p2, c);
...
void show_quadratic(point_2d p1, point_2d p2, point_2d p3, rgb c = RGB_RED, bool nav = true)
{
	_test_ds[TEST_DS_OUT_GFX_1]->log_quadratic(p1, p2, p3, c);

	if (nav) {
		_test_win[TEST_WIN_OUT]->cursor = p3;
		explore_outp(*_test_win[TEST_WIN_IN], *_test_win[TEST_WIN_OUT], 13);
	}
}
...
void mess(double num, char* text = NULL)
{
	char in_text[1000];
